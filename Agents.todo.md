...
- [ ] **[PRIO: hoch] GUI-FILMSTRIP-DUP-1** User-Befund 2026-09-03: Filmstrip doppelt + zwei Thumbnails desselben Bildes (linke Schiene vs. Filmstrip vs. Panel-Thumb — hängt mit RIGHT-THUMB-1 zusammen, hier separat: jede Bild Miniatur genau einmal pro Ansicht, Auswahl-Sync über alle). Abnahme: headless Count-Test + Vision-Re-Check.
- [ ] **[PRIO: hoch] GUI-NAV-RECT-1** User-Befund 2026-09-03: Viewport-Rechteck im Navigator-Thumbnail bildet den sichtbaren Ausschnitt falsch ab (Lage/Größe stimmt nicht mit Preview überein). Fix: Rechteck aus `roi_from_zoom` + Pan-Offset rechnen (Single Source of Truth mit Preview-Placement), Regressionstest mit Zoom+Pan-Matrix (Fit/100%/Custom × Pan). Abnahme: headless Rect-vs-ROI-Test + Vision-Re-Check.
- [ ] **[PRIO: hoch] GUI-PREVIEW-NOISE-1** User-Befund 2026-09-03 (kritisch): Haupt-Preview zeigt bei Zoom Fit nur graues Rauschen statt Bildinhalt, während Navigator-Thumbnail korrekt ist (2 Screenshots, landscape + portrait). Diagnose: ROI/Placement vs. Draft-Render (falsche Region/Skala aus Textur gelesen?). Abnahme: headless Test (Fit-Preview Pixel-Histogramm ≈ Thumbnail, kein Rausch-Flat), `cargo test -p lumina-gui` grün.
- [ ] **[PRIO: hoch] GUI-SIDECAR-RESTORE-1** User-Befund 2026-09-03: Basic-Regler wirken nach Neustart/Bildwechsel nicht aus Sidecar wiederhergestellt (Datei enthält Werte — Exposure −0.62 etc. — Anzeige fraglich ob aus Datei oder Session). Abnahme: Reopen-Regressionstest (Sidecar→alle Basic-Slider + Preview wenden Werte an), DoD-§1-Anker.
- [ ] **[PRIO: hoch] GUI-OPTICS-1** User-Befund 2026-09-03: Optics-Sektion zeigt nur „(unset)"-Zeilen (k1–k3, Vignette, CA), keine sichtbare Korrekturwirkung, unverständliche Parameternamen. Diagnose: Lensfun-Profil vorhanden/angewendet oder stillschweigend inaktiv (dann Verstoß gegen Kein-stiller-Fallback)? Fix: sichtbarer Profilstatus (Name oder „kein Profil — Korrektur inaktiv") + verständliche Labels/Tooltips. Abnahme: headless Status-Test + Vision-Re-Check.
- [ ] **[PRIO: hoch] GUI-TOAST-OVERLAP-1** Vision-Befund User-Screenshot 2026-09-03 (erweitert: Toast verschwindet nicht von selbst): grüner „Vorschau bereit"-Toast überdeckt Thumbnail in linker Schiene (Klick blockiert, Überlappung) und bleibt stehen. Fix: Toast als Overlay ohne Layout-Breite + Auto-Dismiss (Timeout) + manueller Schließen-Button. Abnahme: headless Overlap-/Dismiss-Test + Vision-Re-Check, `cargo test -p lumina-gui` grün.
- [ ] **[PRIO: hoch] GUI-ZOOM-CUSTOM-1** Vision-Befund User-Screenshot 2026-09-03: Zoom-Anzeige steht direkt nach Laden auf „Custom" statt „Fit" (SOLL: Fit Default; Custom nur nach Pan/Zoom). Diagnose + Fix + headless Test.
- [ ] **[PRIO: hoch] GUI-RIGHT-THUMB-1** Vision-Befund User-Screenshot 2026-09-03: identisches Thumbnail 3× sichtbar (linke Schiene, Filmstrip, rechtes Panel über Presets). Klären welches Panel-Thumb beabsichtigt ist (F-100), Duplikat entfernen oder begründen. Abnahme: Vision-Re-Check.
- [ ] **[PRIO: niedrig] GUI-LIBRARY-BADGE-CONTRAST-1** Vision-Nebenbefund Badge-Golden: Pfad-Badge dunkler Text auf dunklem Grund (lesbar, aber kontrastarm). Fix: hellerer Hintergrund/Schrift. Abnahme: Vision-Re-Check.
- [ ] **[PRIO: hoch] AGENT-HARNESS-1** Interner Agent-Harness Kern (F-100/F-103, `lumina-gui` exklusiv, kein `tokio`/WebSocket im Prod-Binary): `AgentHarness`-Wrapper um `egui_kittest::Harness` (headless wgpu, 1280×800, `pixels_per_point=1`) mit `wait_quiescent()` (Preview-Generation stabil, kein `pending_full_render`), `ui_tree_json()` (AccessKit-Serialisierung + Custom-Ids), `snapshot_rgba()` (On-Demand GPU-Readback) + CPU-`ColorImage`-Pfad. Abnahme: headless Tests für Harness-APIs, `cargo test -p lumina-gui` grün, `clippy -D warnings`, `fmt --check`.
- [ ] **[PRIO: hoch] AGENT-HARNESS-2** Semantische AccessKit-Knoten (F-100, `lumina-gui` exklusiv): Preview-Viewport (Bild-ID + Generation), Filmstrip-Item (Dateipfad als Label), Histogramm (Mean/Median-Text), Navigator-Rechteck, alle Buttons/Slider per Id auffindbar. Abnahme: jeder Klick-Target aus `ui_tree_json()` auflösbar, Headless-Tests + kittest 10/10 + Vision-Loop.
