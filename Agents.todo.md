...
| 1.0 | GUI-INSTRDBG-17c | alle G | Letzte Buttons + Eyedropper |
...
- [ ] **[PRIO: mittel] GUI-INSTRDBG-17c (Release: 1.0)** Letzte user-sichtbare Schaltflächen instrumentieren (Fortsetzung von 17b-REST, gleiche Mechanik): Spot-Distraction-Checkboxen (`set_spot_distraction`), Red-Eye-Pick-Toggle (`set_red_eye_pick_mode`), Presets-Refresh (`reload_preset_entries`), WB-Eyedropper, Punktfarbe Add/Remove, generative Canvas-Buttons. Pflicht wie 17b-REST (Variante + Makro als erste Anweisung, `ALL_GUI_ACTIONS` + Tests erweitern, Klick-Test je Button, Debug-only, Release-Nachweis, Ratchet). Abnahme: alle user-sichtbaren Buttons instrumentiert (H-1 geschlossen), Gates debug+release grün.
