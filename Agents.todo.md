...
20 offene Tasks (Checkbox-Zählung dieser Datei) — Block A: 16,
...
| 1.0 | LENSFUN-GATE-2 | G-10 | Lensfun-Gate-Follow-ups |
...
- [ ] **[PRIO: hoch] GPU-RENDER-PARITY-1 (Release: 1.0)** Volle GPU-Parität (User-Entscheid 2026-09-12, Agents.md Änderungsregeln): alle Renderstufen in `lumina-gpu` implementieren, sodass kein Rezept mehr wegen `unsupported_gpu_stages_for` auf CPU zurückfällt — CPU-Routing aus diesem Grund ist ein Fail, kein Zustand. Stand: Stufe 1 (Ton) + Stufe 2 (Detail) + Red-Eye + Spot-Heal + SourceAction-Batching + volle Eintritts-Validierung verifiziert BESTANDEN. Ziel 2026-09-13 (User-Entscheid): KEIN CPU-only-Zweig — JEDE gültige Rezept-Konfiguration läuft auf GPU (CPU bleibt Referenz + No-Adapter-Fallback, kein GPU-only). ERLEDIGT und committet 2026-09-14 (beidseitig BESTANDEN re-verifiziert; SOLL in `feature/architecture/pipeline.md` nachgezogen): Geometrie-Welle (geometry/perspective/lens_correction inkl. exakter Maße + lautem VRAM-Refusal) + GUI-LENSFUN-GATE-1 (Corrector-Kontext + Memo + deterministischer Test, CLI-Reason-Mirror). NOCH OFFEN: generative_edit. Erledigt 2026-09-14 (BESTANDEN): Lens-Blur-Welle (WGSL-Pass, Parität byte-identisch/≤1) + Camera-WB-Welle (valider As-Shot-Kontext wird per `set_camera_white_balance` getragen + Oracle-identisch validiert, nur invalide Gains routen laut auf CPU; CLI/MCP/GUI binden; 18 Kombis byte-identisch, gebunden==ungebunden). Nebenbefunde: Perf-Pooling; Coverage-Lücken (Rotation 180/270/negativ, VRAM-Refusal nur für Crop getestet). **User-Entscheid 2026-09-13 (Muss): GPU-Default + Notfall-CPU** — alles GPU-fähig (Gate leer für gültige Rezepte); CPU bleibt Referenz und Fallback nur bei fehlendem/defektem Adapter, kein GPU-only-Produktpfad. Lebendes Inventar: `cpu_routing_inventory_is_complete` (`crates/lumina-gpu/tests/parity.rs`) — jede Reason-Klasse konstruiert + geflaggt. Scope: Stufen in `lumina-gpu` (+ Tests), GUI-Integration danach; Parität je Stufe per CPU-Oracle-Test (byte-identisch oder dokumentierte Toleranz). Abnahme: leere Unsupported-Liste für Standardrezepte, GPU-Gates lokal grün (Metal), kein GPU-only-Weg (CPU-Referenz bleibt). Crate: `lumina-gpu`.
...
- [ ] **[PRIO: niedrig] PARITY-PATHS-2 (Release: 1.0)** Follow-ups aus PARITY-PATHS-Verifizierung (BESTANDEN, Vision 3/3): Bounds auf `diff==0` verschärfen wo 0 gemessen (neutral/detail, derzeit Bounded 1/2); SKIP-Zweig real auf adapterloser Maschine ausführen (hier nur Code-Review, Metal vorhanden); kittest-Parity-Zelle mit aktivem Lensfun-Corrector (CPU-Route) ergänzen. Abnahme: kittest grün. Crate: `lumina-gui`.
- [ ] **[PRIO: mittel] LENSFUN-GATE-2 (Release: 1.0)** Follow-ups aus Kombi-Re-Verifizierung (BESTANDEN): `routing_fallback_reason()`/Badge nennt den präzisen Grund (inkl. Lensfun-Reason und VRAM-Maß-Refusal — derzeit nur generisch/leer, observability-nah an „kein stiller Fallback"); Lensfun-Cache/Routing an aktuell angezeigte Dims binden (Single-Slot-Teilung Preview/Export, Near-Identity-Kippfall). Abnahme: GUI-headless + kittest grün. Crate: `lumina-gui`.
