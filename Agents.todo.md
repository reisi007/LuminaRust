...
21 offene Tasks (Checkbox-Zählung dieser Datei) — Block A: 17,
...
- [ ] **[PRIO: hoch] GEN-EXPAND-CACHE-1 (Release: 1.0)** User-Bedingung 2026-09-14 zum Generative-Entscheid: der einmalige Expand ist ok, darf aber NICHT bei jedem Rendering neu laufen (BFS blockiert sonst jedes Render). Expand-Ergebnis cachen/wiederverwenden mit Rezept-/Quell-abhängiger Invalidierung (Identität analog AI-Masken: Quelle, Geometrie, Seed, Canvas); `zdata` kind=2 + Rezept-Link nutzen. Abnahme: Zweit-Render ohne BFS (Test pinnt Cache-Hit/Miss/Invalidierung), kein stiller Fallback (stale Cache nie still serviert). Crates: `lumina-core` (+ `lumina-gui`/`lumina-cli` für Anbindung).
