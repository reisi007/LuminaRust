...
- [ ] **[PRIO: hoch] R5-STACKVIS-21 (User-Order 2026-09-20, nach Sort-Mini-Welle)** Stack-Markierung visuell von der Selektions-Umrandung entkoppeln (User: „kein Rechtsklick" = keine Kontextmenüs, nur Buttons — bereits Stand, bleibt so; Stack-Zugehörigkeit braucht eigenes Zeichen statt blauem Rahmen, z. B. Stapel-Optik/eigene Farbe + 1/n-Badge behalten). Abnahme: headless Paint-Test (Stack-Farbe ≠ Selection-Farbe) + kittest-Golden-Rebaseline mit Vision-Review + unabhängige Verifizierung.
- [ ] **[PRIO: hoch] R5-DENOISEFIX-22 (User-Order 2026-09-20, nach Sort-Mini-Welle)** Sample-Fixture `sample-data/raw/aircraft-landscape.cr3.lumina.json`: `denoise_ai.enabled: true` mit `pending-integration`-Gewichten erzwingt CPU-Route + harten Error im Full-Res-Pfad (Manual-2 belegt) → auf `false` setzen (Doku-Vermerk; kein Code-Strip, G-1-WGSL-Pass bleibt). Abhängige prüfen, GPU-Routen-Badge danach weg. Abnahme: betroffene Tests grün + unabhängige Verifizierung.
...
- [ ] **[PRIO: hoch] R5-MASKVIS-25 (User-Order 2026-09-20, nach Sort-Mini-Welle)** Masken-Overlay-Sichtbarkeit entkoppelt vom Panel-Zustand (Toggle in der Toolbar über dem Bild, auch bei geschlossenem Masking-Panel) + Bildbereich vergrößern (Seiten-Panels ausblendbar für maximale Preview). Abnahme: headless Tests (Toggle ohne Panel, Panel-Hide) + Golden + unabhängige Verifizierung.
