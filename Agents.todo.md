...
- [ ] **[PRIO: hoch] GUI-HISTOGRAM-1** Echtes Histogramm als Grafik (256-Bin-Luminanz, Painter-Kurve, P01/P99-Marker, einklappbar, raus aus Top-Panel). Plan: `docs/plans/gui-preview-histogram-zoom-2026-09-04.md`. Abnahme: headless Bins→Plot-Test + kittest Golden, `cargo test -p lumina-gui` grün, clippy/fmt/wasm grün.
- [ ] **[PRIO: hoch] GUI-PREVIEW-NAV-1** Navigator mit Viewport-Rechteck + Drag-Pan, einklappbar, Zoomstufen Fit/25/50/75/100/200/FitWidth (Fit Default), Wheel nur mit Modifier. Plan: `docs/plans/gui-preview-histogram-zoom-2026-09-04.md`. Abnahme: headless Zoom-/Pan-Roundtrip-Tests + kittest Golden, `cargo test -p lumina-gui` grün.
- [ ] **[PRIO: hoch] GUI-SLIDER-SAVE-1** ALLE Slider auditieren + Slider-Commit speichert Sidecar + INFO-Log (Debounce-Ende → render_full → save_sidecar, Fehler laut). Audit-Umfang: Basic/Kurve/HSL/Presence/Detail/Effects/Optics/Geometry/Masken-Dichte/Local-Adjustments/Spot-Radius-Feather-Opacity (direkte Feldzuweisung vs set_*/mark_dirty vs save_sidecar je Slider klassifizieren, Lücken schließen). Testlücken-Analyse (`docs/plans/gui-preview-histogram-zoom-2026-09-04.md` §Testpflicht): warum Headless-Tests das nicht fingen (Recipe-Assert statt Sidecar-Assert, Debounce-Pfad zeitbasiert ungetestet, Goldens pixel- statt persistenzbasiert) + Integrations-Test Slider→Debounce→Sidecar→Reload als Regression. Plan: `docs/plans/gui-preview-histogram-zoom-2026-09-04.md`. Abnahme: headless tempdir-Test je Slider-Klasse (Sidecar existiert + Wert drin), `cargo test -p lumina-gui` grün.
