...
- [ ] **[PRIO: hoch] GEN-FILL-01** Auto-Fill Transparent nach Lens Correction generativ (Doku `feature/product/generative-expand.md` auto_fill_transparent, `docs/plans/gap-generative-fill-transparent-2026-09-02.md` L-01): transparente Randpixel nach `LensCorrection`/`Perspective` automatisch per Inpaint füllen, Trigger transparente Pixel nach `apply_geometry`, implizite Maske, Modell `inpaint` lokal (ONNX) vs Cloud getrennt, `auto_fill_transparent` bool default false Teil von `recipe_hash`/`RenderKey`/Identität, kein stiller Fallback (`stale`/`missing` sichtbar), visuell `kittest`+Histogram `PSNR>35dB`/byte-identisch bei Seed-Pin. Tests: `lumina-core` Pipeline + `lumina-onnx` Inpaint + `lumina-gui` headless visuell.
- [ ] **[PRIO: hoch] GEN-FILL-02** Manueller Expand per Checkbox default „auf Bild beschneiden" (`feature/product/generative-expand.md` expand_beyond_image, Gap L-02): Checkbox `expand_beyond_image` false= crop auf Bild, true= Canvas größer ziehen (Rahmen ziehen, `output_* > source_*`, `source_offset` Translation), Pipeline `Perspective → GenerativeEdit(expand) → Crop`, Teil von `recipe_hash`, visuell `kittest` Golden + `preview_generation` bump. Tests: `lumina-core` Canvas-Geometrie + `lumina-sidecar` Schema + `lumina-gui` UI.
- [ ] **[PRIO: hoch] GEN-FILL-03** Crop-Entscheidung behalten vs zuschneiden (`keep_generative_content`, Gap L-03): nach Crop entscheiden ob abgeschnittene generative Inhalte behalten (`keep=true` Canvas bleibt) oder auf aktuelle Ansicht zuschneiden (`keep=false` materialisiert `canvas=crop_rect`, `source_offset - crop_offset` Translation, verkürzt Canvas), Teil von Identität, kein Datenverlust still, visuell Golden. Tests: `lumina-core`/`lumina-sidecar`/`lumina-gui`.
