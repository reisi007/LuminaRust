...
| LRPAR-G14-DENOISE-20 | KI-Denoise (G-14, Release 2.0): additive, optionale Stufe `recipe.adjustments.denoise_ai` (DenoiseAI → F-096 → F-095, `None`/`enabled:false`/`strength:0` = Identität) + deterministischer Blend/Kachel-Assemblierung + §6-Statusmodell (`unavailable`/`stale`/`missing`/`corrupt`, kein stiller Fallback); Produzenten-Provenienz im `extras`, zdata `kind = "denoise_rgb"`; `denoise`-ONNX-Capability (`pending-integration` → `unavailable`), CLI `denoise`, GUI-Panel/Badges + F-074-N8-Denoise-Benchmark/Budget. Offen: echte Gewichte (S6/F-078), F1-Blend-Digest, F3-Restfelder | [KI-Denoise-Entscheid](decisions/LRPAR-G14-DENOISE-20.md) | niedrig |
