...
| LRPAR-G14-DENOISE-20 | KI-Denoise (G-14, Release 2.0): additive, optionale Stufe `recipe.adjustments.denoise_ai` (DenoiseAI → F-096 → F-095, `None`/`enabled:false`/`strength:0` = Identität) + deterministischer Blend/Kachel-Assemblierung + §6-Statusmodell (`unavailable`/`stale`/`missing`/`corrupt`, kein stiller Fallback); Produzenten-Provenienz im `extras`, zdata `kind = "denoise_rgb"`; GPU-Pfad routet aktive Stufen laut auf die CPU zurück. Offen: ONNX-Verdrahtung, CLI/GUI, Perf F-074 | [KI-Denoise-Entscheid](decisions/LRPAR-G14-DENOISE-20.md) | niedrig |
| LRPAR-G09-CULL-25 | KI-Culling (G-09, Release 2.5): Assisted Culling als Vorschlag-Sichtung (kein Auto-Rating); Stufe 1 heuristisch im eigenen Crate `lumina-cull` (deterministisch, modellfrei), Stufe 2 ONNX-gated | [KI-Culling](decisions/LRPAR-G09-CULL-25.md) | niedrig |
