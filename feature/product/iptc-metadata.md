...
- `lumina meta copy <quelle> [--fields <id,…>] [--out <clipboard-datei>]`
  ...
  **Clipboard-Format** (`--out`, Default s. u.): `{ "format":
  ...
  `--out`-Bundleschutz (META-COPYPASTE-2, CLI-GUARD-HARDLINK-1): `--out` darf nie das Original,
  dessen `.lumina.json`/`.lumina.zdata` oder einen Hardlink darauf
  treffen (`reject_protected_output` wie Export, Exit 1, nichts geschrieben;
  Hardlink-Erkennung via `(dev,inode)`, nur Unix).
