...
2. **Schnell (heuristisch):** **Size-Slider** in der Tool-Leiste (Größe in Quellpixeln, `1..=512`, sichtbar als px-Wert) plus `[`/`]`-Alias; Feather/Opacity optional. Der **Kreis-Cursor** am Zeiger zeigt die Live-Größe (`radius × aktuelle Ansichts-Skala`). **Klick auf das Bild** setzt einen Spot (Dab); die Quelle wird automatisch per deterministischem Offset gewählt (`1.5 × Radius` zur Bildmitte hin, `spot_auto_offset`), optional Alt-Ziehen (Post-MVP). Vorschau heilt **instant** (kein Job, kein Modell). Der Dab persistiert die Rezeptparameter über `commit_spot_heal` (`info!`-Log, Sidecar-Save); „Rückgängig" entfernt Spot (`Clear spots`).
