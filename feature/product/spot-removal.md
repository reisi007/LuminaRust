...
**Stand 2026-09-20 (R5-DUST-23, implementiert, Verifizierung NICHT BESTANDEN 2026-09-20 — Blocker B1: Feather/Opacity bei 1024×720 geclippt, Rework offen):** Das Staub-Werkzeug ist aus der Sidebar in die Toolbar über dem Bild gezogen (Arming per Toolbar-Icon „Heal“ / `Q` als Alias; kein Sidebar-Abschnitt mehr). Die Tool-Optionen liegen in einer toolbar-nahen Leiste über dem Bild (Size-Slider sichtbar + `[`/`]`-Alias, Feather/Opacity, Quick/Generativ; G-04-Extras in der Gruppe „Remove options“). Der bis dahin fehlende Pointer-Pfad ist ergänzt: Klick auf das Bild setzt einen heuristischen Dab (Auto-Source-Offset, `commit_spot_heal`), ein Kreis-Cursor zeigt die Live-Größe (`radius × Ansichts-Skala`). Headless-Tests: Arming, Size→Dab-Radius, Dab→Heilung→Reload, `[`/`]`, Cursor; kittest-Goldens `develop_spot_tool_options[_expanded]`. Vorher:
...
1. **Werkzeug wählen:** „Staub entfernen" (Spot-Heal) wird über das **Toolbar-Icon „Heal" über dem Bild** armiert (`Q` ist nur Alias) — es gibt **keinen Sidebar-Abschnitt** mehr. Die Tool-Optionen liegen in der toolbar-nahen Leiste über dem Bild: Toggle **Schnell** (Default) vs. **Generativ**; Schnell ist Lightroom-ähnlich (Kreis/Spot, sofort), Generativ zeigt Modell-Badge (`inpaint_heal` verfügbar/nicht verfügbar). Die seltenen G-04-Extras (Visualize/Detect/Distraction/Variant) liegen in der Gruppe „Remove options“.
2. **Schnell (heuristisch):** **Size-Slider** in der Tool-Leiste (Größe in Quellpixeln, `1..=512`, sichtbar als px-Wert) plus `[`/`]`-Alias; Feather/Opacity optional. Size/Feather/Opacity liegen panel-breiten-sicher in zwei kompakten Zeilen und bleiben bei 1024×720 **je mit Label** sichtbar (kein Überlauf in das Histogramm-Panel). Der **Kreis-Cursor** am Zeiger zeigt die Live-Größe (`radius × aktuelle Ansichts-Skala`). **Klick auf das Bild** setzt einen Spot (Dab); die Quelle wird automatisch per deterministischem Offset gewählt (`1.5 × Radius` zur Bildmitte hin, `spot_auto_offset`), optional Alt-Ziehen (Post-MVP). Vorschau heilt **instant** (kein Job, kein Modell). Der Dab persistiert die Rezeptparameter über `commit_spot_heal` (`info!`-Log, Sidecar-Save); „Rückgängig" entfernt Spot (`Clear spots`).
...
- **Vorher/Nachher:** `Y`-Toggle (Before/After) hält Rezept unverändert, toggelt nur View-Flag + generational bump; kittest-Snapshots für die toolbar-nahe Spot-Tool-Leiste (Schnell vs. Generativ, Badge-Zustände).
...
- **Visuelle Analyse automatisch (je Modus):** Golden-Image für heuristisch (byte-identisch) + generativ (PSNR/SSIM-Gate mit Seed-Pin), Histogram-Digest Delta (1/256-Toleranz), kittest Snapshots für die Spot-Tool-Leiste (Schnell/Generativ/Badge), Before/After (`Y`) Toggle hält Rezept unverändert.
