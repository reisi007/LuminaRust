...
use lumina_core::{CoreError, LensfunMap, MemoryBudget};
use lumina_sidecar::{AspectPreset, Crop, LensCorrection, Perspective};

use crate::geometry_shaders::{
    CaParams, CropParams, LensParams, MirrorParams, PerspectiveParams, RotateParams, CA_STAGE_SRC,
    CROP_STAGE_SRC, LENS_STAGE_SRC, MIRROR_STAGE_SRC, PERSPECTIVE_STAGE_SRC, ROTATE_STAGE_SRC,
};
use crate::shaders;
...
pub(crate) struct GeometryPipelineState {
    ...
    /// GPU-LENSFUN-PARITY-1 resample pass (own layout: uniform + input + coord/
    /// blue/gain textures).
    pub lensfun: crate::lensfun::LensfunPipelineState,
}
...
pub(crate) fn build_geometry_pipelines(
    ...
    Ok(GeometryPipelineState {
        ...
        lensfun: crate::lensfun::build_lensfun_pipeline(device)?,
        layout,
    })
}
...
pub(crate) enum GeometryStep {
    ...
    /// GPU-LENSFUN-PARITY-1: resample using the caller-bound
    /// [`lumina_core::LensfunMap`] (CPU-precomputed warp/gain map) in place of
    /// the manual distortion/vignette model. Dimension-preserving: the map
    /// carries the input dimensions.
    Lensfun { out_width: u32, out_height: u32 },
    Crop {
        params: CropParams,
        out_width: u32,
        out_height: u32,
...
impl GeometryPlan {
    ...
    pub(crate) fn from_recipe_with(
        ...
        // GPU-LENSFUN-PARITY-1: a caller-bound map is dimension-preserving and
        // replaces the manual lens step. A dimension mismatch, or a distortion
        // corrector without an explicit crop (which would trigger the CPU
        // oracle's content-based default crop), is a loud error — the caller
        // must render it through the full CPU reference instead of producing
        // divergent pixels. The TCA flag skips the manual CA step exactly like
        // the oracle (`lensfun_tca_active`).
        let tca_active = lensfun.is_some_and(LensfunMap::has_tca);
        if let Some(map) = lensfun {
            if (map.width, map.height) != (width, height) {
                return Err(invalid(
                    "lensfun_map.dimensions",
                    f64::from(map.width),
                    f64::from(width),
                    f64::from(width),
                ));
            }
            if map.has_distortion
                && recipe
                    .geometry
                    .as_ref()
                    ...
                    .is_none()
            {
                return Err(invalid("lensfun_map.default_content_crop", 0.0, 0.0, 1.0));
            }
        }

        if lensfun.is_some() {
            // The map already covers distortion + vignetting (+ TCA); no manual
            // lens coefficients are applied (the oracle ignores them too).
            steps.push(GeometryStep::Lensfun {
                out_width: cw,
                out_height: ch,
            });
