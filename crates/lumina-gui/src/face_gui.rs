...
// Debug-only (GUI-INSTRDBG-17c-Rest): the `instrument_gui_action!` call site
// expands to nothing in release, so `GuiAction` is only needed under
// `debug_assertions` (avoids an unused-import warning in the release gate).
use crate::i18n::Str;
...
impl LuminaApp {
    ...
    pub fn create_face_mask(
        ...
        instrument_gui_action!(self, GuiAction::CreateFaceMask);
        let name = name.into();
    ...
    pub(crate) fn current_face_analysis(&self) -> Result<FaceAnalysis, GuiError> {
        self.face_analysis()
            .cloned()
...
pub(crate) mod tests {
    ...
    pub(crate) fn seed_face(app: &mut LuminaApp) {
