...
pub enum GuiAction {
    ...
    CombineMasks,
    DuplicateMask,
    // R5-BRUSH-24 follow-up: the per-mask management row is user-visible
    // and mutates the sidecar/session selection, so it gets the same
    // exhaustive instrumentation as the older masking actions.
    RenameMask,
    DeleteMask,
    MoveMask,
    GroupDuplicateMask,
...
impl GuiAction {
    ...
    pub const fn name(self) -> &'static str {
        match self {
            ...
            GuiAction::RenameMask => "rename_mask",
            GuiAction::DeleteMask => "delete_mask",
...
#[path = "gui_action_list.rs"]
mod gui_action_list;

pub use gui_action_list::ALL_GUI_ACTIONS;
